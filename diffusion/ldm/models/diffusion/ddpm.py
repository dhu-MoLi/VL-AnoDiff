...
class LatentDiffusion(DDPM):
    ...
    def p_losses(self, x_start, cond, t, mask=None, noise=None):
        ...
        size = model_output.shape[3]
        mask = F.interpolate(mask, size=(size,size), mode='bilinear', align_corners=False)
        mask = torch.where(mask != 0, torch.tensor(1.).to(self.device), torch.tensor(0.).to(self.device))

        # loss_simple = self.get_loss(model_output, target, mean=False).mean([1, 2, 3])
        loss_simple = (self.get_loss(model_output, target, mean=False) * mask).mean([1, 2, 3])
        loss_dict.update({f'{prefix}/loss_simple': loss_simple.mean()})

        # logvar_t = self.logvar[t].to(self.device)
        # Modified - ensure t and self.logvar are on the same device
        t_cpu = t.cpu() if t.device.type != 'cpu' else t
        ...
        loss_vlb = (self.get_loss(model_output, target, mean=False) * mask).mean(dim=(1, 2, 3))
